...
export function useEmployeeAllowances(userId: string | null, periodId: string | null) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const { data } = await (supabase
        .from('employee_allowances') as any)
        .select('*')
        .eq('user_id', userId)
  ...
  const toggleAllowance = useCallback(async (key: AllowanceKey) => {
    ...
    await (supabase.from('employee_allowances') as any).update({ is_enabled: newEnabled }).eq('id', current.id);
  }, [allowances]);
  ...
  const updateAllowance = useCallback(async (
    ...
    await (supabase.from('employee_allowances') as any).update(updates).eq('id', current.id);
  }, [allowances]);
